...
| 路径 | 说明 |
|------|------|
| `D:/workspace/mobile_game_reports/reports/` | 历史月报存档目录，命名格式：`<产品名>_<平台>月度复盘 - YY年M月.md`（如 `DEMO_CASINO_安卓月度复盘 - 26年2月.md`） |
| `D:/workspace/mobile_game_reports/skill/section_registry.json` | 章节定义、触发字段、分析要点 |
| `D:/workspace/mobile_game_reports/skill/csv_mapping.json` | CSV 文件名 → 章节映射配置（看板搭建完成后填入） |
| `D:/workspace/mobile_game_reports/skill/style_guide.json` | 文风规范、禁止写法、措辞参考 |
| `D:/workspace/mobile_game_reports/skill/wip_report.md` | 当前月报草稿（逐章追加写入） |

---

## 第零步：初始化

1. 从用户输入中判断三个要素：**产品名**（如 Demo Casino）、**平台**（iOS / Android / 全平台）、**报告月份**（如 `26年3月`）
2. 根据平台裁剪章节列表：
...
3. 自动推算上月文件名，用 Read 工具读取上月 md：
   - 优先查找产品专属文件（如 `DEMO_CASINO_安卓月度复盘 - <上月>.md`）
   - 若不存在，尝试通用文件（如 `SAMPLE_PORTFOLIO月度复盘 - <上月>.md`）
...
dest = r'D:/workspace/mobile_game_reports/data/<产品名>_<月份>'
...
**PowerShell：**
```powershell
$env:PYTHONUTF8=1; python D:/workspace/mobile_game_reports/skill/scripts/run_all.py `
  --data-dir "<actual_csv_dir>" `
  --month "<本月YYYY-MM>" `
  --prev-month "<上月YYYY-MM>" `
  --output D:/workspace/mobile_game_reports/skill/extracted_data.json
...
```bash
PYTHONUTF8=1 python ~/workspace/mobile_game_reports/skill/scripts/run_all.py \
  --data-dir "<actual_csv_dir>" \
  --month "<本月YYYY-MM>" \
  --prev-month "<上月YYYY-MM>" \
  --output ~/workspace/mobile_game_reports/skill/extracted_data.json
```
...
5.  付费礼包监控            → 内购数据_分点位 CSV      → platform: all  ⚠️ 用户数/ARPPU 不可跨点位汇总（重复计数），汇总层取基础数据其二
...
3. 将最终报告从 `wip_report.md` 复制到正式存档：
   ```
   D:/workspace/mobile_game_reports/reports/<产品名>_<平台>月度复盘 - <月份>.md
   ```
