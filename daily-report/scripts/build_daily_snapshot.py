...
def date_windows_str(target):
    """Return (d, d_s, d1_s, d7_s) for a target date string.

    d      — normalized datetime for target
    d_s    — target formatted as YYYY-MM-DD
    d1_s   — target minus 1 day
    d7_s   — target minus 7 days
    """
    d = pd.to_datetime(target).normalize()
    return (
        d,
        date_str(d),
        date_str(d - pd.Timedelta(days=1)),
        date_str(d - pd.Timedelta(days=7)),
    )


def avg_7day_before(d, value_fn):
    """Return the mean of value_fn(date_str(dt)) for the 7 days before d, ignoring None."""
    vals = []
    for dt in pd.date_range(d - pd.Timedelta(days=7), d - pd.Timedelta(days=1)):
        v = value_fn(date_str(dt))
        if v is not None:
            vals.append(v)
...
def parse_spin(csv_path, target):
    ...
    for r in rows:
        rows_by_date[clean_date_str(r.get("时间", ""))].append(r)
    ...
    d, d_s, d1_s, d7_s = date_windows_str(target)
...
def parse_iap_daily(csv_path, target):
    ...
    metrics = {
        ...
        # NOTE: iap_users / iap_arppu 来自分点位 CSV 跨点位求和，
        # 同一用户在多个点位购买会被重复计数，不可用于汇总展示。
        # 汇总层的付费用户数和 ARPPU 应取 core（基础数据其二）。
        # 以下仅保留供内部参考，报告中不得展示。
        "_iap_users_raw": build_comparison(cur_u, d1_u, d7_u, avg7_u),
        "_iap_arppu_raw": build_comparison(cur_a, d1_a, d7_a, avg7_a),
    }
...
def parse_promo_daily(csv_path, target, anchor_offset_days=0):
    ...
    for mk, keyword in PROMO_METRIC_MAP.items():
        ...
        avg7 = avg_7day_before(d, lambda dt_s, kw=keyword: get_val(kw, dt_s))
...
def parse_paying_users(csv_path, target):
    ...
    for out_name, cfg in metrics_def.items():
        def agg(day, _cfg=cfg):
            vals = day_metric(_cfg["names"], day)
            if not vals:
                return None
            if _cfg["agg"] == "sum":
...
def _parse_resource_date_series(dict_rows, header_date_map, date_cols, target):
    """Parse resource CSV in date-series format (newer format)."""
    ...
    return {
        "period": {"current": cur_day, "previous": d1_day},
...
def _parse_resource_single_period(rows, headers):
    """Parse resource CSV in single-period format (原因, 分析指标, <period>)."""
...
def _parse_resource_dual_period(rows, headers, compare_col):
    """Parse resource CSV in dual-period compare format (duplicate column names)."""
    ...
    return {
        "period": {"current": cur_period, "previous": prev_period},
        "consume_total": {
...
def parse_resource(csv_path, target=None):
    ...
    Dispatches to one of three private helpers based on the CSV format:
    - date_series: newer format with date columns (daily report standard)
    - single_period: fallback with only one period column
    - dual_period: legacy format with duplicate column names and 对比时间 separator
    """
    with open(csv_path, encoding="utf-8-sig") as f:
    ...
    if dict_rows and date_cols:
        return _parse_resource_date_series(dict_rows, header_date_map, date_cols, target)
    ...
    compare_col = next((i for i, h in enumerate(headers) if "对比时间" in str(h)), None)
    if compare_col is None:
        if len(headers) == 3 and "分析指标" in headers and "原因" in headers:
            return _parse_resource_single_period(rows, headers)
        raise ValueError("'对比时间' column not found in paying_user_resource")

    return _parse_resource_dual_period(rows, headers, compare_col)
...
def parse_and_track(coverage, cov_key, result, result_key, parse_fn, warnings, *args, **kwargs):
    """Run a single-result parse function and update result / coverage / warnings."""
    try:
        p = coverage[cov_key]["file"]
        if not p:
            warnings.append(f"{result_key}: {cov_key} CSV not found")
        else:
            result[result_key] = parse_fn(p, *args, **kwargs)
            mark_parsed(coverage[cov_key], result_key)
    except Exception as e:
        warnings.append(f"{result_key}: {e}")
        mark_error(coverage[cov_key], result_key, e)
...
def main():
    ...
    parse_and_track(coverage, "basic_data_2", result, "core", parse_core, warnings, target)
    ...
    parse_and_track(coverage, "basic_data_1", result, "basic1", parse_basic1, warnings, target)
    parse_and_track(coverage, "paying_user_summary", result, "paying_users", parse_paying_users, warnings, target)
    parse_and_track(coverage, "paying_user_resource", result, "resource", parse_resource, warnings, target=target)
    parse_and_track(coverage, "big_r_detail", result, "big_r", parse_big_r, warnings)
    parse_and_track(coverage, "ad_board_data", result, "ads", parse_ads_daily, warnings, target)
